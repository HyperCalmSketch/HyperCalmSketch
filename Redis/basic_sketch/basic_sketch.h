...
public:
    ...
    const size_t len() const { return length; }
    const char *c_str() const { return s; }
    const int to_int() const { return atoi(s); }
    const long long to_long_long() const { return atoll(s); }
    const double to_double() const { return atof(s); }
};
