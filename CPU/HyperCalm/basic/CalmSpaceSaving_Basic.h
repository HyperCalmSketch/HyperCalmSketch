...
class CalmSpaceSaving_Basic : public CalmSpaceSavingTopK
{
    struct Last_Time{
        ...
        int count;
        Last_Time(float _last_batch = 0, float _last_item = 0, int _count = 0):
                last_batch(_last_batch), last_item(_last_item), count(_count){}
    };
protected:
...
public:
	CalmSpaceSaving_Basic(double _time_threshold, double _unit_time,
        ...
        printf("w = %d\t (Length of the SS in CalmSS)\n",capacity);
	...
	bool insert(uint32_t key, float time, int bf_new, int freq = 1) {
		...
		if (itr == hash_table_circular.end()) {
            ...
            itr->second = Last_Time(time, time, itr->second.count);
            insert_calm({key, delta}, freq);
        }
        return 1;
	}
